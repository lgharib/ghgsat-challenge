...
class TestTargetCase(TestCase):
    ...
    def test_api_targets_search_matching_bounding_box(self):
        data = {
            "bounding_box": "POLYGON((-74.150848 45.265222, -73.355713 45.790509, -73.355713 45.265222, -74.150848 45.265222, -74.150848 45.265222))"}
...
class TestObservationCase(TestCase):
    def setUp(self):
        ...
        Observation.objects.create(
            ...
            timestamp=datetime(2007, 12, 6, 16, 29, 43,
                               79043, tzinfo=pytz.UTC),
            target=target1)

        Observation.objects.create(
            image_url="http://image-server/observations/LAT20LONG10202001202.png",
            timestamp=datetime(2019, 12, 1, 16, 29, 43,
                               79043, tzinfo=pytz.UTC),
            target=target2)

        Observation.objects.create(
            image_url="http://image-server/observations/LAT20LONG10202001203.png",
            timestamp=datetime(2019, 12, 5, 8, 0, 0, 79043, tzinfo=pytz.UTC),
            target=target2)

        Observation.objects.create(
            image_url="http://image-server/observations/LAT20LONG10202001203.png",
            timestamp=datetime(2019, 12, 10, 9, 0, 0, 79043, tzinfo=pytz.UTC),
            target=target2)

        Observation.objects.create(
            image_url="http://image-server/observations/LAT20LONG10202001204.png",
            timestamp=datetime(2019, 12, 5, 9, 0, 0, 79043, tzinfo=pytz.UTC),
            target=target3)
    ...
    def test_api_observations_search_matching_bounding_box_and_time_period(self):
        data = {"bounding_box": "POLYGON((-74.150848 45.265222, -73.355713 45.790509, -73.355713 45.265222, -74.150848 45.265222, -74.150848 45.265222))",
                "start_timestamp": "2019-12-01T00:00:00Z",
                "end_timestamp": "2019-12-06T23:59:59Z",
                }
        response = self.client.get(
            "/search_observations/", data, format='json')
